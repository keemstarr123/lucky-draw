  ...
  return (
    <div className="h-[100vh] w-full bg-white relative overflow-hidden font-sans text-slate-900 flex flex-col">
      ...
      <main className="flex-1 flex flex-col relative pt-[80px]">
        {/* Hero */}
        <div className="z-20 flex flex-col items-center justify-center px-4 text-center shrink-0 mb-4 md:mb-6">
          <motion.div
          ...
          >
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-display font-bold leading-tight bg-clip-text text-transparent bg-gradient-to-r from-slate-900 via-slate-700 to-slate-900 drop-shadow-sm max-w-4xl">
              Spin the Lucky Draw – Win Back Your Loyalty
            </h1>
            <p className="text-sm md:text-base text-slate-600 max-w-xl">
              Corporate customers at risk of churning can spin to unlock
              exciting rewards and stay with us.
            </p>

            {/* CTA */}
            <motion.button
              whileHover={{ scale: isSpinning || !!winner ? 1 : 1.05 }}
              whileTap={{ scale: isSpinning || !!winner ? 1 : 0.96 }}
              onClick={handleStartSpin}
              disabled={isSpinning || !!winner}
              className={`relative group overflow-hidden px-10 py-4 rounded-full font-bold text-lg shadow-xl shadow-orange-500/20 transition-all
                ${
                  isSpinning || winner
        ...
        {/* Cards row (Nintendo-style) */}
        <div className="flex-1 flex items-start justify-center">
          <div className="w-full max-w-6xl mx-auto mt-4 md:mt-6 flex justify-center">
            <SpinWheel
